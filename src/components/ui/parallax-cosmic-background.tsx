...
const CosmicParallaxBg: React.FC<CosmicParallaxBgProps> = ({
  ...
  return (
    <div className={`cosmic-parallax-container ${className}`}>
      ...
      {/* Title and subtitle */}
      {head.trim() ? <div id="title">{head.toUpperCase()}</div> : null}
      {textParts.filter(Boolean).length ? (
        <div id="subtitle">
          {textParts.filter(Boolean).map((part, index) => (
            <React.Fragment key={index}>
              <span className={`subtitle-part-${index + 1}`}>{part.toUpperCase()}</span>
              {index < textParts.filter(Boolean).length - 1 && ' '}
            </React.Fragment>
          ))}
        </div>
      ) : null}
