...
function Dock() {
  return (
    <div className="relative flex items-center gap-2 scale-90 sm:gap-4 sm:scale-95">
      <div className="flex items-center gap-3 rounded-[28px] bg-gradient-to-r from-black/90 via-[#071a33]/90 to-[#0f2f55]/90 px-3 py-2 shadow-2xl shadow-black/50 ring-1 ring-amber-200/18 backdrop-blur-lg sm:gap-5 sm:rounded-[48px] sm:px-6 sm:py-3">
...
function DockIcon({
  ...
  return (
    <a
      className="hover-halo group relative grid h-12 w-12 place-items-center rounded-xl ring-1 ring-amber-200/20 bg-gradient-to-b from-amber-200/8 via-[#12345c]/42 to-black/78 backdrop-blur-xl shadow-lg shadow-black/35 transition-transform duration-200 hover:-translate-y-1 hover:scale-[1.05] hover:ring-amber-200/45 sm:h-14 sm:w-14"
      href={href}
      aria-label={label}
      target={href.startsWith("http") ? "_blank" : undefined}
      rel={href.startsWith("http") ? "noreferrer" : undefined}
    >
      <Icon className={`dock-icon dock-icon--${tone} h-5 w-5 text-slate-200 transition-transform duration-200 group-hover:scale-110`} strokeWidth={2.1} />
      <span className="tooltip pointer-events-none absolute -bottom-6 translate-y-1/2 text-[9px] tracking-wide text-slate-200/80 sm:text-[10px]">
        {label}
      </span>
    </a>
  );
